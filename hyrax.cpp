...
G1 perdersen_commit_redundant(G1* g,Fr* f,ll* id,int n,int m) // compute MSM on g[i]*f[id[i]] for i=0~n-1, id[i] is in [0,m-1], n>> m
{
    G1 ret;
    ret.clear();
    G1* base2=new G1[m];
...
Pack bullet_reduce(G1 gamma, Fr*a,Fr* x,Fr y,G1*g,G1& G,int n,bool need_free) // length n
{
    ...
    gamma_minus1=G*x1a2+perdersen_commit_classic(g+n/2,x,n/2);
    gamma_1=G*x2a1+perdersen_commit_classic(g,x+n/2,n/2);
    Fr c,invc;
...
void ll_commit_worker(G1*& Tk,G1*& g, ll*& w,int rownum,int colnum,G1*& W)
{
    ...
    while (true)
    {
            ...
            for(int i=0;i<colnum;i++)
                f1[i]=w[idx+i*rownum];
            Tk[idx]=perdersen_commit(g,f1,colnum,W);
            endq.Push(idx);
    }
}
G1* prover_commit(ll* w, G1* g, int l,int thread_n) //compute Tk, int version with pippenger
{
    ...
    for(int i=0;i<thread_n;i++)
    {
        workers.emplace_back(ll_commit_worker,std::ref(Tk),std::ref(g),std::ref(w),rownum,colnum,std::ref(W[i]));
    }
    for (auto& t : workers)
    {
        t.join();
...
void fr_commit_worker(G1*& Tk,G1*& g, ll*& w,Fr* f,int m,int rownum,int colnum)
{
    ...
    while (true)
    {
            ...
            for(int i=0;i<colnum;i++)
                f1[i]=w[idx+i*rownum];
            Tk[idx]=perdersen_commit_redundant(g,f,f1,colnum,m);
            endq.Push(idx);
    }
}
G1* prover_commit_fr(ll* w, Fr* f,int m,G1* g, int l,int thread_n) 
{
  ...
  //  t.start();
    ...
    for(int i=0;i<thread_n;i++)
    {
        workers.emplace_back(fr_commit_worker,std::ref(Tk),std::ref(g),std::ref(w),std::ref(f),m,rownum,colnum);
    }
    for (auto& t : workers)
    {
        t.join();
...
void fr_commit_worker_general(G1*& Tk,G1*& g, Fr*& w,int rownum,int colnum)
{
    ...
    while (true)
    {
            ...
            for(int i=0;i<colnum;i++)
                f1[i]=w[idx+i*rownum];
            Tk[idx]=perdersen_commit_classic(g,f1,colnum);
            endq.Push(idx);
    }
}
G1* prover_commit_fr_general(Fr* w, G1* g, int l,int thread_n)
{
  ...
  //  t.start();
    ...
    for(int i=0;i<thread_n;i++)
    {
        workers.emplace_back(fr_commit_worker_general,std::ref(Tk),std::ref(g),std::ref(w),rownum,colnum);
    }
    for (auto& t : workers)
    {
        t.join();
...
Fr prover_evaluate(ll*ww ,Fr*r, int l)  
{
 //   timer t(true);
 //   t.start();
    Fr eval=0;
...
void open(ll*w,Fr*r,Fr eval,G1&G,G1*g,G1*comm,int l)
{
    ...
    G1 tprime=perdersen_commit_classic(comm,L,rownum); //random combine comm
